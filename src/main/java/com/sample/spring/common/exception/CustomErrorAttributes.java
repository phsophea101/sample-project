...
public class CustomErrorAttributes extends DefaultErrorAttributes {
    ...
    public Map<String, Object> getErrorAttributes(WebRequest webRequest, ErrorAttributeOptions options) {
        ...
        error.put(SystemType.PATH.getValue(), path);
        error.put(SystemType.DESCRIPTION.getValue(), message);
        if ((exception instanceof Exception)) {
            error.put(SystemType.DESCRIPTION.getValue(), ((Exception) exception).getLocalizedMessage());
            error.put(SystemType.EXCEPTION.getValue(), ((Exception) exception).getClass().getSimpleName());
        } else if (HttpStatus.NOT_FOUND.value() == status)
            error.put(SystemType.EXCEPTION.getValue(), InvalidEndpointRequestException.class.getSimpleName());
        else if (HttpStatus.UNAUTHORIZED.value() == status)
            error.put(SystemType.EXCEPTION.getValue(), UnauthorizedClientException.class.getSimpleName());
        if (!error.get(SystemType.DESCRIPTION.getValue()).toString().endsWith("."))
            error.put(SystemType.DESCRIPTION.getValue(), error.get(SystemType.DESCRIPTION.getValue()) + ".");
