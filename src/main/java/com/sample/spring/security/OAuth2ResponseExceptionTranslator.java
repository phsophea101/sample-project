...
//        response.put(SystemType.PATH.getValue(), responseEntity.getBody().);
        ...
        if (ex instanceof FatalSecurityException) {
            error.put(SystemType.DESCRIPTION.getValue(), ((FatalSecurityException) ex).getDescription());
            error.put(SystemType.PATH.getValue(), ((FatalSecurityException) ex).getDescription());
            ...
            if ("Token has expired".equals(ex.getMessage()) || (StringUtils.isNotEmpty(ex.getMessage()) && ex.getMessage().contains("Access token expired")))
                error.put(SystemType.DESCRIPTION.getValue(), "Access token has expired");
                ...
                error.put(SystemType.DESCRIPTION.getValue(), "Client " + clientId + " not found");
            } else
                error.put(SystemType.DESCRIPTION.getValue(), "Invalid access token");

        } else if (ex instanceof InvalidGrantException)
            error.put(SystemType.DESCRIPTION.getValue(), "Unauthorized access");
        else if (ex instanceof BadCredentialsException)
            error.put(SystemType.DESCRIPTION.getValue(), "Incorrect username or password");
        else if (ex instanceof OAuth2Exception) {
            OAuth2Exception e = (OAuth2Exception) ex;
            error.put(SystemType.DESCRIPTION.getValue(), e.getMessage());
            ...
            error.put(SystemType.DESCRIPTION.getValue(), ex.getMessage());
        error.put(SystemType.DESCRIPTION.getValue(), exception);
        error.put(SystemType.MESSAGE.getValue(), message);
        if (ContextUtil.isProfileTesting())
            response.put(SystemType.ERROR.getValue(), error);
        return ResponseEntity.status(HttpStatus.valueOf(status)).body(new OAuth2ExceptionResponse(response));
    }
}
